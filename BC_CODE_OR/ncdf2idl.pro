...
;;DATA TO IDL SAVED FILES. THESE ARE EASIER TO PROCESS.
;;
;; FILENAME: ncdf2idl.pro
...
function ncdf2idl,in,out,SWITCH_1d,varname,multfactor,NUMLANDPOINTS,land
;varname=VNAME
;
...
pfile   = in
print,'input file= ',pfile
outfile = out
print,'output file= ',outfile
; opening the model data.
id=NCDF_OPEN(pfile)
; the variable 'varname' is read from the file specified and stored in ************************
; the idl variable dataarray

varInfo = NCDF_VarInq(id, varname)
dimIDs = varInfo.dim
...
NCDF_CLOSE,id
;
IF (ndims EQ 4) THEN dataarray=reform(dataarray(*,*,0,*))
IF (ndims EQ 5) THEN dataarray=reform(dataarray(*,*,0,0,*))
