...
IF (min(tas_c) LT 0.0) THEN BEGIN
    print,'Model has negative temperature!', min(tas_c),where(min(tas_c) LT 0.0) 
    errors=1
ENDIF
IF (min(tmax_c-tas_c) LT 0.0) THEN BEGIN
    ...
    print,tmax_c(bb(0),bb(1)),tas_c(bb(0),bb(1))
    help,bb(0),bb(1)
    print,'correcting the data...'
    tmax_c(bb[0,*],bb[1,*])=tas_c(bb[0,*],bb[1,*])
    print,tmax_c(bb(0),bb(1)),tas_c(bb(0),bb(1))
    errors=1
    print,'Rewriting '+pfiletmax
    save,filename=pfiletmax,tmax_c
ENDIF
IF (min(tmax_c-tas_c) LT 0.0) THEN STOP ;Checking again...
...
IF (max(tmin_c-tas_c) GT 0.0) THEN BEGIN
    ...
    print,tmin_c(bb(0),bb(1)),tas_c(bb(0),bb(1))
    print,'correcting the data...'
    tmin_c(bb[0,*],bb[1,*])=tas_c(bb[0,*],bb[1,*])
    print,tmin_c(bb(0),bb(1)),tas_c(bb(0),bb(1))
    errors=1
    print,'Rewriting '+pfiletmin
    save,filename=pfiletmin,tmin_c
ENDIF
IF (min(tmin_c-tas_c) GT 0.0) THEN STOP ;Checking again...

IF (min(tmax_c-tmin_c) LT 0.0) THEN BEGIN
    cc=tmax_c-tmin_c
    aa=where(cc LT 0.0,count)
    print,'Model has negative diurnal range in',count,' points!(',count*100./N_ELEMENTS(cc),' %)'
    print,tmin_c(bb(0),bb(1)),tmax_c(bb(0),bb(1))
    print,'correcting the data...'
    tmin_c(bb[0,*],bb[1,*])=tas_c(bb[0,*],bb[1,*])
    tmax_c(bb[0,*],bb[1,*])=tas_c(bb[0,*],bb[1,*])
    print,tmin_c(bb(0),bb(1)),tmax_c(bb(0),bb(1))
...
;
;ALE            T=tas_e(n,ind)
;ALE            Td=Tmax_e(n,ind)-Tmin_e(n,ind)
            T=tas_c(n,ind)
            Td=Tmax_c(n,ind)-Tmin_c(n,ind)
;ALE            IF (Td GT 0.0) THEN Tf=(tas_e(n,ind)-Tmin_e(n,ind))/Td
