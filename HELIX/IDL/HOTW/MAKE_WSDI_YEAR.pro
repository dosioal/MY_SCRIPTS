...
;===============================================
FILEO=DIR_SM+'/'+VAR+'_'+SCEN+'_'+STRMID(YEAR_INI,4,4)+'-'+STRMID(YEAR_END,4,4)+'_'+STAT+'.dat'
print,'====================================='
...
;TXx
;=============================================
DIR_TXx=DIR_HOME+RUN(ir)+'/'+VAR+'/PP/TXx/'
...
help,work
VAR_TXx_25p=REFORM(WORK(*,*,0:nt2-1))
ENDIF ELSE BEGIN
nx=dims_a(0)
ny=dims_a(1)
nt2=dims_a(3)
...
NCDF_VARGET,fileID,VAR,WORK
help,work
VAR_TXx_25p=REFORM(WORK(*,*,0,0:nt2-1))
ENDELSE
NCDF_CLOSE, fileID
help,VAR_TXx_25p
...
window,0,retain=2
!p.multi=[0,2,2]
contour,VAR_TXx_25p(*,*)-273.15,xstyle=1,ystyle=1,/fill,levels=FINDGEN(10)+34.
...
NCDF_VARGET,fileID,VAR,WORK
help,work
VAR_TXx_75p=REFORM(WORK(*,*,0:nt2-1))
ENDIF ELSE BEGIN
nx=dims_a(0)
ny=dims_a(1)
nt2=dims_a(3)
...
NCDF_VARGET,fileID,VAR,WORK
help,work
VAR_TXx_75p=REFORM(WORK(*,*,0,0:nt2-1))
ENDELSE
NCDF_CLOSE, fileID
help,VAR_TXx_25p
VAR_TXx_75p=REVERSE(VAR_TXx_75p,2)

contour,VAR_TXx_75p(*,*)-273.15,xstyle=1,ystyle=1,/fill,levels=FINDGEN(10)+34.,title='Txx_75p'
contour,landmask,/OVERPLOT,levels=[0,1],color=0

contour,VAR_TXx_75p(*,*)-VAR_TXx_25p(*,*),xstyle=1,ystyle=1,levels=[1],/OVERPLOT,title='Txx_75p-Xx_25p'
contour,landmask,/OVERPLOT,levels=[0,1],color=0
...
;==============================================
;PERCENTILE
DIR_PERC=DIR_HOME+RUN(ir)+'/'+VAR+'/PERC/'
FILE_PERC=VAR+'_p90_1976-2005.nc'  ;CLIMATOLOGICAL PERCENTILES
FILE_PERC_SHIFT=VAR+'_p90_1976-2005_shift.nc'  ;CLIMATOLOGICAL PERCENTILES
...
contour,VAR_90p(*,*,0)-273.15,xstyle=1,ystyle=1,/fill,levels=FINDGEN(10)+34.
contour,landmask,/OVERPLOT,levels=[0,1],color=0
...
FILE_RUN=VAR+'_'+SCEN+'.nc'  ;CLIMATOLOGICAL PERCENTILES
FILE_SHIFT=VAR+'_'+SCEN+'_shift.nc'  ;CLIMATOLOGICAL PERCENTILES
...
IF SCEN NE 'hist' THEN BEGIN 
spawn,'cdo mergetime '+DIR_RUN+'YEAR/test_2006.nc '+DIR_WORK+'/test_2006.nc '+DIR_WORK+'/test_2006_full.nc'
spawn,'cdo mv '+DIR_WORK+'/test_2006_full.nc '+DIR_WORK+'/test_2006.nc'
ENDIF
spawn,'mv '+ DIR_WORK+'/test_*.nc '+DIR_RUN+'YEAR/'
ENDIF
ENDIF ELSE BEGIN ;CLIMATOLOGICAL ANOMALY
FILE_RUN=VAR+'_'+SCEN+'.nc'  ;CLIMATOLOGICAL PERCENTILES
FILE_AN=VAR+'_'+SCEN+'_an.nc'  ;CLIMATOLOGICAL ANOMALY
FILE_EX=FINDFILE(DIR_RUN+FILE_AN,count=count)
IF count EQ 0 THEN spawn,'cdo sub '+DIR_RUN+FILE_RUN+' '+DIR_PERC+FILE_PERC+' '+DIR_RUN+FILE_AN
FILE_SHIFT=VAR+'_'+SCEN+'_an_shift.nc'  ;CLIMATOLOGICAL PERCENTILES
FILE_EX=FINDFILE(DIR_RUN+FILE_SHIFT,count=count)
...
IF SCEN NE 'hist' THEN BEGIN 
spawn,'cdo mergetime '+DIR_RUN+'YEAR/test_2006_an.nc '+DIR_WORK+'/test_2006_an.nc '+DIR_WORK+'/test_2006_an_full.nc'
spawn,'cdo mv '+DIR_WORK+'/test_2006_an_full.nc '+DIR_WORK+'/test_2006_an.nc'
ENDIF
spawn,'mv '+ DIR_WORK+'/test_an_*.nc '+DIR_RUN+'YEAR/'
ENDIF
ENDELSE

IF SCEN EQ 'hist' THEN YI=FIX(STRMID(YEAR_INI,4))+1  ELSE YI=FIX(STRMID(YEAR_INI,4)) ;WHEN SHIFTED STARTS FROM YI+1
;================================
;YEAR LOOP
;================================
...
contour,VAR_SM(*,*,10),xstyle=1,ystyle=1,/fill,levels=FINDGEN(21)*.4-4
contour,landmask,/OVERPLOT,levels=[0,1],color=0

iyyy=YI-FIX(YEAR_INI)
PRINT,'YEAR=',YI,YI-FIX(YEAR_INI)
...
WINDOW,4+ir,RETAIN=2,title=RUN(ir),xsize=1800,ysize=1000
!p.multi=[0,7,5]
;FOR iy=0,NYEAR-1 DO BEGIN &$
FOR iy=0,34 DO BEGIN &$
CONTOUR,landmask,xstyle=1,ystyle=1,title=STAT+' '+STRMID(YEAR_INI+iy,4),charsize=2,xrange=[300,500],yrange=[230,350] &$
CONTOUR,WSDI_Y(*,*,iy),levels=levels,/fill,/OVERPLOT,c_colors=colors &$
...
!p.color=0
print,ic
WINDOW,4+ic,RETAIN=2,title=RUN(ir)
!p.multi=[0,2,2]
IF SCEN NE 'hist' THEN BEGIN &$
...
colors=[80,100,150,190,210,250]
IF STAT EQ 'HWMId' THEN lev_c=STRCOMPRESS(STRMID(levels,4,5)) ELSE lev_c=STRCOMPRESS(STRMID(levels,4,4))
CONTOUR,landmask,xstyle=1,ystyle=1,title=STAT+' '+STRMID(YEAR_INI+1,4)
CONTOUR,WSDI_Y(*,*,1),levels=levels,/fill,/OVERPLOT,c_colors=colors
CONTOUR,landmask,/overplot

CONTOUR,landmask,xstyle=1,ystyle=1,title=STAT+' '+STRMID(YEAR_INI+10,4)
CONTOUR,WSDI_Y(*,*,10),levels=levels,/fill,/OVERPLOT,c_colors=colors
CONTOUR,landmask,/overplot

CONTOUR,landmask,xstyle=1,ystyle=1,title=STAT+' '+STRMID(YEAR_INI+20,4)
CONTOUR,WSDI_Y(*,*,20),levels=levels,/fill,/OVERPLOT,c_colors=colors
CONTOUR,landmask,/overplot

CONTOUR,landmask,xstyle=1,ystyle=1,title=STAT+' '+STRMID(YEAR_INI+30,4)
CONTOUR,WSDI_Y(*,*,30),levels=levels,/fill,/OVERPLOT,c_colors=colors
CONTOUR,landmask,/overplot
...
print,'HWMId HISTORIC VALUES'
print,'USA 1980 ',MAX(wsdi_y(150:200,230:260,9))
print,'EU 2003 ',MAX(wsdi_y(350:370,265:280,32))
;print,'EA 2007 ',MAX(wsdi_y(380:410,260:280,28))
