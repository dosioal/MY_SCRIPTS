...
POP_CDF_M_TOT=FLTARR(NX,NY,NBINS,3,NRUN)
KS_POP_TOT=FLTARR(NX,NY,NBINS,3,NRUN)*!VALUES.F_NaN
...
FILEO_POP='./DATA/POP_HWMId_WORLD_'+RUN(ir)+'_'+SSP+'_ONCE_YEAR.dat'
...
NYEARS=30
POP_CDF_YEAR=FLTARR(NX,NY,NBINS,NYEARS,3) ;1980-2100
...
WORK_HH=FLTARR(NX,NY) &$
IF ib EQ 0 THEN print,'YEAR ',nt0(it),YEAR_INI(0)+nt0(it)+iyy,nt0(it)+iyy &$
WORK=REFORM(WSDI_Y_T(*,*,iyy+nt0(it))) &$ ; *POP_INT_NEW(*,*,iyy+nt0(it)) &$  ; 1976 CHECK!!!
...
POP_CDF_YEAR(*,*,ib,iyy,it)=WORK_HH*REFORM(POP_INT_NEW(*,*,iyy+nt0(it))) &$
ENDFOR &$  ;YEAR iyy
ENDFOR &$ ;bins
ENDFOR  ;


KS_POP=FLTARR(NX,NY,NBINS,3)*!VALUES.F_NaN
;KOLMOGOROV FOR SELECTED HWMDI 
print,'KOLMOGOROV'
FOR ib=0,NBINS-1 DO BEGIN &$
FOR ix=0,NX-1 DO BEGIN &$
FOR iy=0,NY-1 DO BEGIN &$
IF landmask(ix,iy) GT 0.5 THEN BEGIN &$
ww_ref=REFORM(POP_CDF_YEAR(ix,iy,ib,*,0)) &$
ww_1=REFORM(POP_CDF_YEAR(ix,iy,ib,*,1)) &$
ww_2=REFORM(POP_CDF_YEAR(ix,iy,ib,*,2)) &$
ks2,ww_1,NYEARS,ww_ref,NYEARS,stat_v,prob_v,fn1,fn2,sor1,sor2  &$;1.5C -REF
KS_POP(ix,iy,ib,0)=prob_v &$
ks2,ww_2,NYEARS,ww_ref,NYEARS,stat_v,prob_v,fn1,fn2,sor1,sor2 &$; 2C -REF
KS_POP(ix,iy,ib,1)=prob_v &$
ks2,ww_2,NYEARS,ww_1,NYEARS,stat_v,prob_v,fn1,fn2,sor1,sor2  &$; 2C - 1.5C
KS_POP(ix,iy,ib,2)=prob_v &$
ENDIF &$
ENDFOR &$
ENDFOR &$
ENDFOR ;bins
...
;MAKE TIME MEAN
POP_CDF_M=FLTARR(Nx,NY,NBINS,3)
FOR it=0,2 DO BEGIN &$
FOR ib=0,NBINS-1 DO BEGIN &$
FOR ix=0,NX-1 DO BEGIN &$
FOR iy=0,NY-1 DO BEGIN &$
IF landmask(ix,iy) GT 0.5 THEN BEGIN &$
WORK=REFORM(POP_CDF_YEAR(ix,iy,ib,*,it)) &$
WORK(WHERE(WORK EQ 0))=!VALUES.F_NaN &$
POP_CDF_M(ix,iy,ib,it)=MEAN(WORK,/NaN) &$
ENDIF &$
ENDFOR &$
ENDFOR &$
ENDFOR &$
...
save,filename=FILEO_POP,bins,POP_CDF_YEAR,KS_POP,POP_CDF_M
...
;POP_CDF_YEAR=1
POP_CDF_M_TOT(*,*,*,*,ir)=POP_CDF_M
KS_POP_TOT(*,*,*,*,ir)=KS_POP
ENDIF
...
print,POP_CDF_M_TOT(520,220,8,1,*)

POP_CDF_MED=FLTARR(NX,NY,NBINS,3)
;MODELS MEDIAN
FOR ix=0,Nx-1 DO BEGIN &$
FOR iy=0,Ny-1 DO BEGIN &$
IF landmask(ix,iy) GT 0.5 THEN BEGIN  &$
...
WORK=REFORM(POP_CDF_M_TOT(ix,iy,*,it,ir)) &$
WORK(WHERE(FINITE(WORK,/NaN) EQ 1))=0 &$
POP_CDF_M_TOT(ix,iy,*,it,ir)=WORK &$
ENDFOR &$
FOR ib=0,NBINS-1 DO BEGIN &$
POP_CDF_MED(ix,iy,ib,it)=MEDIAN(POP_CDF_M_TOT(ix,iy,ib,it,*)) &$
ENDFOR &$
ENDFOR &$
ENDIF &$
ENDFOR &$
ENDFOR
print,POP_CDF_M_TOT(520,220,8,1,*)
print,POP_CDF_MED(520,220,8,1)

MOD_AGR_KS=FLTARR(NX,NY,NBINS,3)
THRES=0.05
FOR ix=0,Nx-1 DO BEGIN &$
FOR iy=0,Ny-1 DO BEGIN &$
IF landmask(ix,iy) GT 0.5 THEN BEGIN  &$
...
FOR ib=0,NBINS-1 DO BEGIN &$
FOR ir=0,NRUN -1 DO BEGIN &$
IF KS_POP_TOT(ix,iy,ib,it,ir) LE THRES THEN MOD_AGR_KS(ix,iy,ib,it)=MOD_AGR_KS(ix,iy,ib,it)+1  &$
ENDFOR &$
ENDFOR &$
ENDFOR &$
ENDIF &$
...
w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
		...
		title='Models median 1976-2005 HWMId > '+STRING(BINS(ib)),/CURRENT)
c_plot= CONTOUR(POP_CDF_MED(*,*,ib,0),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2)

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,2],MARGIN=MARGIN, $
		title='Models median +1.5 HWMId>24',/CURRENT)
c_plot= CONTOUR(POP_CDF_MED(*,*,ib,1),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2)
s_plot= CONTOUR(MOD_AGR_KS(*,*,ib,0),lon,lat,c_value=[4,7],/OVERPLOT,C_FILL_PATTERN=pattern_r,/FILL,COLOR="black",GRID_UNITS=2)

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,3],MARGIN=MARGIN, $
		title='Models median +2 HWMId>24',/CURRENT)
c_plot= CONTOUR(POP_CDF_MED(*,*,ib,2),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2)
s_plot= CONTOUR(MOD_AGR_KS(*,*,ib,1),lon,lat,c_value=[4,7],/OVERPLOT,C_FILL_PATTERN=pattern_r,/FILL,COLOR="black",GRID_UNITS=2)

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,4],MARGIN=MARGIN, $
		title='Models median +2-1.5 HWMId>24',/CURRENT)
c_plot= CONTOUR(POP_CDF_MED(*,*,ib,2)-POP_CDF_MED(*,*,ib,1),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2)
s_plot= CONTOUR(MOD_AGR_KS(*,*,ib,2),lon,lat,c_value=[4,7],/OVERPLOT,C_FILL_PATTERN=pattern_r,/FILL,COLOR="black",GRID_UNITS=2)


FOR ir=0,NRUN-1 DO BEGIN &$
w1=WINDOW(WINDOW_TITLE=STRING(RUN(ir)),DIMENSIONS=[1000,700]) &$
w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,1],MARGIN=MARGIN, $
		title='1976-2005 HWMId > '+STRING(BINS(ib)),/CURRENT) &$
c_plot= CONTOUR(POP_CDF_M_TOT(*,*,ib,0,ir),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2) &$

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,2],MARGIN=MARGIN, $
		title='Models median +1.5 HWMId>24',/CURRENT) &$
c_plot= CONTOUR(POP_CDF_M_TOT(*,*,ib,1,ir),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2) &$

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,3],MARGIN=MARGIN, $
		title='Models median +2 HWMId>24',/CURRENT) &$
c_plot= CONTOUR(POP_CDF_M_TOT(*,*,ib,2,ir),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2) &$

w_plot= image(landmask, GRID_UNITS=2,IMAGE_LOCATION=[-180,-90],IMAGE_DIMENSIONS=[360,180], $
	        TRANSPARENCY=93,MAP_PROJECTION='Robinson',GRID_LATITUDE=0,GRID_LONGITUDE=0, $
	        FONT_SIZE=10,FONT_STYLE=1,AXIS_STYLE=1,LAYOUT=[2,2,4],MARGIN=MARGIN, $
		title='Models median +2-1.5 HWMId>24',/CURRENT) &$
c_plot= CONTOUR(POP_CDF_M_TOT(*,*,ib,2,ir)-POP_CDF_M_TOT(*,*,ib,1,ir),lon,lat,c_value=levels,RGB_INDICES=colors,/FILL,RGB_TABLE=rgb,/OVERPLOT,GRID_UNITS=2) &$
ENDFOR
