...
;
;       C_COLORS:  Setting this keyword to an array of color indexes the colorbar 
;                 will be filled only with the colors specified in this array 
;
; COMMON BLOCKS:
;       None.
;
...
PRO MY_COLORBAR, BOTTOM=bottom, CHARSIZE=charsize, COLOR=color, DIVISIONS=divisions, LEVELS=lev_c,$
...
ENDIF ELSE BEGIN
   ...
   print,'messagge from the routine COLORBAR'
   print,'you cannot specify both ncolors and c_colors keywords' 
   STOP
   ENDIF
ENDELSE 
...
;IF N_ELEMENTS(lev_c) EQ 0 THEN lev_c = ''
...
IF KEYWORD_SET(vertical) THEN BEGIN
  ...
  ENDIF ELSE BEGIN
        ...
        FOR kk=0,tmp(1)-1 do begin
          FOR jj=0,nlin-1 do begin
              bytmp(index)=BYTE(c_colors(kk))
              index=index+1
          ENDFOR
         WHILE index LT dint*(kk+1) DO BEGIN
              IF index GT 255 THEN goto,pippo1
              bytmp(index)=BYTE(c_colors(kk))
              index=index+1
         ENDWHILE
        ENDFOR
pippo1:
        bytmp(255)=c_colors(tmp(1)-1)
         bar= REPLICATE(1B,10) # bytmp    
         IF N_ELEMENTS(position) EQ 0 THEN position = [0.88, 0.15, 0.95, 0.95]
  ENDELSE
ENDIF ELSE BEGIN
  IF N_elements(c_colors) EQ 0 then BEGIN
...
;    IF N_ELEMENTS(position) EQ 0 THEN position = [0.15, 0.88, 0.95, 0.95]
     IF N_ELEMENTS(position) EQ 0 THEN position = [0.1, 0.95, 0.9, 0.99]
  ENDIF ELSE BEGIN
     tmp=size(c_colors)
        nlin=FIX(256./tmp(1))
        dint=256./tmp(1)
        ...
        FOR kk=0,tmp(1)-1 do begin
          ...
          ENDFOR
         WHILE index LT dint*(kk+1) DO BEGIN
              IF index GT 255 THEN goto,pippo
              bytmp(index)=BYTE(c_colors(kk)) 
              index=index+1
         ENDWHILE
        ENDFOR
pippo:
        bytmp(255)=c_colors(tmp(1)-1)
        bar=  bytmp # REPLICATE(1B,10)
        IF N_ELEMENTS(position) EQ 0 THEN position = [0.1, 0.95, 0.9, 0.99]
  ENDELSE
ENDELSE

   ; Scale the color bar.
IF N_ELEMENTS(c_colors) eq 0 then bar = BYTSCL(bar, TOP=ncolors-1) + bottom
...
IF KEYWORD_SET(vertical) THEN BEGIN
   ...
   IF KEYWORD_SET(right) THEN BEGIN
      ...
      IF N_ELEMENTS(lev_c) EQ 0 THEN BEGIN
      ...
      AXIS, YAXIS=1, YTICKS=divisions, YTICKN=lev_c,$
         YTICKLEN=0.1, YSTYLE=1, COLOR=color, CHARSIZE=charsize
   ...
   ENDIF ELSE BEGIN
      ...
      ENDIF ELSE BEGIN
;	      print,divisions,color,lev_c
  
      PLOT, [1,10], /NODATA, XTICKS=1, YTICKS=divisions, XSTYLE=1, YSTYLE=4, $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
         XTICKFORMAT='(A1)', YTICKLEN=0.1 
     
      AXIS, YAXIS=1, YTICKS=divisions, YTICKFORMAT='(A1)',$
         YTICKLEN=0.1, YTITLE=title, YSTYLE=1, COLOR=color, CHARSIZE=charsize
      AXIS, YAXIS=0, YTICKS=divisions, YTICKNAME=lev_c, $; YTICKFORMAT=format, $
         YTICKLEN=0.1, YTITLE=title, YSTYLE=1, COLOR=color, CHARSIZE=charsize
...
ENDIF ELSE BEGIN
   ...
   IF KEYWORD_SET(top) THEN BEGIN
      ...
      PLOT, [1,10], /NODATA, XTICKS=divisions, YTICKS=1, XSTYLE=9, YSTYLE=1,YRANGE=[1.,10.], $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
      ...
      PLOT, [1,10], /NODATA, XTICKS=divisions, YTICKS=1, XSTYLE=4, YSTYLE=1,YRANGE=[1.,10.], $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
         YTICKFORMAT='(A1)', XTICKLEN=0.1, $
         XTITLE=title

      AXIS, XTICKS=divisions, XTICKFORMAT='(A1)',XSTYLE=1, COLOR=color, CHARSIZE=charsize, $
          XTICKLEN=0.1,  XAXIS=0
      AXIS, XTICKS=divisions, XTICKN=lev_c,XSTYLE=1, COLOR=color, CHARSIZE=charsize, $
          XTICKLEN=0.1,  XAXIS=1
   ...
   ENDIF ELSE BEGIN     
      ...
      PLOT, [1,10], /NODATA, XTICKS=divisions, YTICKS=1, XSTYLE=1, YSTYLE=1,YRANGE=[1.,10.], $
         POSITION=position, COLOR=color, CHARSIZE=charsize, /NOERASE, $
