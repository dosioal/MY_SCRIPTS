...
home='/media/disk/DATA/ENSOBS/ENS_MODELS/'
...
ifile=home+'read_in_'+MODEL_ID(imd)+'_'+APPLICATION_PERIOD_START+'-'+APPLICATION_PERIOD_STOP+'.nc'
print,'READING ',ifile
...
CASE VAR OF
    ...
    ;'Tmin': tas_c=Tn_e
    ;'Tmax': tas_c=Tmax_e
  ;  'Tmin': IF im EQ 0 THEN tas=Tmin_e ELSE tas=idldata
  ;  'Tmax': IF im EQ 0 THEN tas=Tmax_e ELSE tas=idldata
ENDCASE
idldata=0
...
CASE VAR OF
    ...
    'Tmin': tas_c=Tmin_c
    'Tmax': tas_c=Tmax_c
    ;'Tmin': IF im EQ 0 THEN tas_c=Tmin_c ELSE tas_c=idldata
    ;'Tmax': IF im EQ 0 THEN tas_c=Tmax_c ELSE tas_c=idldata
ENDCASE
idldata=0
...
CASE VAR OF
    ...
    'Tmin': obs=idldata
    'Tmax': obs=idldata
    ;'Tmin': IF im EQ 0 THEN obs=Tmin_o ELSE obs=idldata
    ;'Tmax': IF im EQ 0 THEN obs=Tmax_o ELSE obs=idldata
ENDCASE
help,tas,tas_c,obs
...
;help,land_tc
;help,land_o
;print,max(land_t),max(land_tc),max(land_o)

SetIntersection,land_t,land_o,land_t_o
;help,land_t_o
SetIntersection,land_tc,land_o,land_tc_o
;help,land_tc_o
;IF N_ELEMENTS(land_t_o) GT N_ELEMENTS(land_tc_o) THEN land_w=land_tc_o ELSE land_w=land_t_o
;IF im EQ 0 THEN land_com=land_w ELSE setintersection,land_w,land_com,land_com
...
tas_f_y(*,0:NDAYS(im)-1,im)=tas_f
tasc_f_y(*,0:NDAYS(im)-1,im)=tasc_f
obs_f_y(*,0:NDAYS(im)-1,im)=obs_f
print,tas_f(0,0),tas_f(8000,0)
...
obs_f(*,*)=!VALUES.F_NAN
tasc_f(*,*)=!VALUES.F_NAN
tas_f(*,0:NDAYS(im)-1)=tas_f_y(*,0:NDAYS(im)-1,im)
tasc_f(*,0:NDAYS(im)-1)=tasc_f_y(*,0:NDAYS(im)-1,im)
obs_f(*,0:NDAYS(im)-1)=obs_f_y(*,0:NDAYS(im)-1,im)
print,tas_f(0,0),tas_f(8000,0)
...
FOR i=0,NDAYS(im)-1 do begin
tas_com(*,i)=reform(tas_f(land_com,i))
tasc_com(*,i)=reform(tasc_f(land_com,i))
obs_com(*,i)=reform(obs_f(land_com,i))
ENDFOR
...
FOR i=0L,NPOINTS-1 DO BEGIN &$
	 ww=MEAN(tas_com(i,*),/NaN) &$
         tasm2d_f(lon_m(i),lat_m(i))=ww &$
	 ww=MEAN(obs_com(i,*),/NaN) &$
         obsm2d_f(lon_m(i),lat_m(i))=ww &$
	 ww=MEAN(tasc_com(i,*),/NaN) &$
         tascm2d_f(lon_m(i),lat_m(i))=ww &$
ENDFOR
